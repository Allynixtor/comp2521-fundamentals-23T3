...
bool listIsPoggers(struct node *list);
...
int main(void) {
	...
	if (listIsPoggers(list)) {
		printf("POGGERS POGU POG\n");
	} else {
		printf("NOT POG\n");
	}
...
// return true if list is "poggers", false otherwise
bool listIsPoggers(struct node *list) {
	...
	if (list->value % 2 == 0) {
		return list->value < list->next->value && listIsPoggers(list->next);
	} else {
		return list->value > list->next->value && listIsPoggers(list->next);
	}
}
